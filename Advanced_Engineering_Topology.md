...
**Use:** Debug sloppy thinking. Fix strategy documents where metrics are mistaken for goals, or processes for objects **Prompt:** Compile on target: \*\*A board presentation claiming the company's primary strategic goal is to achieve a Net Promoter Score (NPS) of 75 by Q4.\*\* Scan for ontological category errors (Reification, Misplaced Concreteness). For each error, output strictly: ERROR\_TYPE: \[Name\] SOURCE: "\[Exact quote\]" VIOLATION: Expected\_Type=\[X\] vs Actual\_Type=\[Y\] RECOMPILE\_FIX: "\[Rigidly restated sentence\]"
